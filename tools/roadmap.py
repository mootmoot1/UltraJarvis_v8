...
RM = Path("project/roadmap.md")
Q = Path("tasks.txt")
LOG = Path("logs/roadmap.log")


def _log(s: str):
    LOG.parent.mkdir(parents=True, exist_ok=True)
    LOG.write_text(
        (LOG.read_text(encoding="utf-8") if LOG.exists() else "") + s + "\n",
        encoding="utf-8",
    )
...
def seed(limit: int = 0, section: Optional[str] = None) -> dict:
    if not RM.exists():
        return {"ok": False, "error": "project/roadmap.md missing"}
...
def _pop() -> Optional[str]:
    if not Q.exists():
        return None
    lines = [
        line for line in Q.read_text(encoding="utf-8").splitlines() if line.strip()
    ]
    if not lines:
        return None
...
def run(batch: int = 50) -> dict:
    ...
    fails = 0
    total_hint = sum(
        1 for _ in (Q.read_text(encoding="utf-8").splitlines() if Q.exists() else [])
    )
    while n < batch:
        t = _pop()
        if not t:
...
def expand(
    goal: str = "Expansion & hardening", phases: int = 1, items_per_phase: int = 12
) -> dict:
    md = RM.read_text(encoding="utf-8") if RM.exists() else "# Roadmap\n\n"
    last = 0
    ...
    for i in range(1, phases + 1):
        n = last + i
        out.append(f"## Phase {n} — {goal}")
        c = 0
        while c < items_per_phase:
            for b in base:
                if c >= items_per_phase:
                    break
                out.append(f"- {b}")
                c += 1
        out.append("")
...
def phase_status() -> dict:
    ...
    bullets = sum(1 for line in md.splitlines() if line.strip().startswith("- "))
