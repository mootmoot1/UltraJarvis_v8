...
def status()->dict:
    pending=[line for line in (Q.read_text(encoding="utf-8").splitlines() if Q.exists() else []) if line.strip()]
    return {"ok":True,"pending":len(pending)}

def _pop()->Optional[str]:
    if not Q.exists(): 
        return None
    lines=[line for line in Q.read_text(encoding="utf-8").splitlines() if line.strip()]
    if not lines: 
        return None
...
def run(batch:int=50)->dict:
    n=0
    fails=0
    total_hint=sum(1 for _ in (Q.read_text(encoding="utf-8").splitlines() if Q.exists() else []))
    while n<batch:
        t=_pop()
        if not t: 
            break
