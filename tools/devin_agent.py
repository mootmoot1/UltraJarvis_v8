...
def _handle_fake_mode(task: str, run_log: Dict) -> Dict[str, Any]:
    ...
    plan = {
        "files": [{"path": "example.py", "action": "modify", "reason": "fake change"}]
    }
    ...
    try:
        ...
        if file_spec["action"] == "create":
            ...
            if not diff_content.startswith("diff --git"):
                diff_content = _wrap_raw_content_to_diff(
                    file_path, diff_content, is_new=False
                )
...
TOOL_SPEC = {
    ...
    "actions": {"devin": {"help": "Execute natural language task", "run": devin_agent}},
}
