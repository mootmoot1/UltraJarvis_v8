...
import json, shlex, subprocess, sys, platform
from datetime import datetime
from pathlib import Path
from registry import discover_tools, run_tool

MEM_PATH = Path("data/memory.json"); MEM_PATH.parent.mkdir(parents=True, exist_ok=True)
STATE = {"goal": None, "history": []}

def _load_mem():
    global STATE
    if MEM_PATH.exists():
        try: STATE.update(json.loads(MEM_PATH.read_text(encoding="utf-8")))
...
def _speak(text:str):
    ...
    if platform.system().lower()=="darwin":
        try: subprocess.run(["say", text], check=False)
        except Exception: pass
...
def _handle_run_tool(t:str):
    ...
    parts=shlex.split(t)
    if len(parts)<3: return "Usage: run <tool> <action> key=val ..."
...
def _handle_memory(t:str):
    ...
    if tl.startswith("remember "):
        note=t.split(" ",1)[1]; notes=STATE.setdefault("notes",[])
        notes.append({"ts":datetime.utcnow().isoformat()+"Z","note":note}); _save_mem()
        return "Remembered."
    if tl=="show memory":
...
def loop():
    _load_mem()
    _speak(f"Online. Goal: {STATE.get('goal') or '(none)'} | try: set goal <..>, expand phases, seed, run --batch 25, health scan, list tracks, add track content, show memory.")
    while True:
        try: s=input("You: ").strip()
        except EOFError: break
        if not s: continue
        if s.lower() in ("quit","exit"): _speak("Goodbye."); break
        if s.lower().startswith("set goal "): reply=_handle_set_goal(s)
        elif s.lower() in ("expand phases","expand phase","add phase tasks"): reply=_handle_expand()
        elif s.lower().startswith("health"): reply=_handle_health(s)
        elif s.lower().startswith("run "): reply=_handle_run_tool(s)
        elif s.lower().startswith("remember") or s.lower()=="show memory": reply=_handle_memory(s)
            ...
            reply=s  # echo fallback
        _speak(reply); _append_history(s, reply)
