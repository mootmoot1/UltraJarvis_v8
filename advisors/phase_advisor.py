...
def set_goal(goal: str) -> dict:
    d = _load()
    d["goal"] = goal
    d["updated"] = dt.datetime.utcnow().isoformat() + "Z"
    d["history"].append(
        {
...
def status() -> dict:
    d = _load()
    return {"ok": True, "goal": d.get("goal", ""), "last_updated": d.get("updated")}
...
def suggest_tasks(max_items: int = 25) -> dict:
    ...
    buckets = {
        ...
        "GROWTH": [
            ...
            "Partnerships research (10)",
        ],
        "FINANCE": [
            "Pricing hypotheses",
            "Initial P&L skeleton",
...
def append_to_roadmap(items: list) -> dict:
    ...
    if "## Phase Advisor Suggestions" not in md:
