...
def _log(s): LOG.write_text((LOG.read_text() if LOG.exists() else "")+s+"\n", encoding="utf-8")
...
def scan()->dict:
    ...
    try:
        import registry; reg = importlib.reload(registry).discover_tools()
        add("registry.discover_tools", True, f"tools={sorted(reg.keys())}")
    except Exception as e:
        add("registry.discover_tools", False, str(e))
    STATE.write_text(json.dumps({"ts":time.time(),"last_ok":out["ok"],"checks":out["checks"]},indent=2), encoding="utf-8")
...
def plan()->dict:
    tips=[]
    try:
        import registry, importlib; reg = importlib.reload(registry).discover_tools()
        for t in ["files","websearch","browser","os_control","email","automation","roadmap"]:
            if t not in reg: tips.append(f"Add tool '{t}'")
    except Exception:
        tips.append("Registry failing — fix imports")
    if not Path("advisors/phase_advisor.py").exists(): tips.append("Install Phase Advisor")
...
def fix()->dict:
    ...
    for d in ["logs","project","advisors","data","tools","backup"]:
        dp=Path(d); 
        if not dp.exists(): dp.mkdir(parents=True, exist_ok=True); created.append(d)
    _log(f"fix created={created}")
    return {"ok":True,"created":created}

def status()->dict:
    s=STATE.read_text(encoding="utf-8") if STATE.exists() else "{}"
    return {"ok":True,"state":json.loads(s or "{}"),"log_tail":LOG.read_text(encoding="utf-8").splitlines()[-20:] if LOG.exists() else []}
