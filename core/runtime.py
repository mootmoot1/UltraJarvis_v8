...
class RuntimeMemory:
    def __init__(self, runtime_file: str = "data/memory_runtime.json", max_turns: int = 10):
    ...
    def _save_buffer(self):
        try:
            self.runtime_file.write_text(json.dumps(self.buffer, indent=2), encoding="utf-8")
        except Exception as e:
    ...
    def summarize_recent_turns(self, turns: int = 10) -> List[str]:
        ...
        for turn in recent[-3:]:
            ...
            elif any(cmd in user_text.lower() for cmd in ["run", "seed", "status", "health"]):
