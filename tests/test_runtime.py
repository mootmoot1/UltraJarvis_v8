...
def test_speech_queue_timeout():
    ...
    with patch('tools.speech.platform.system', return_value='Darwin'), \
...
def test_watchdog_disable_after_failures():
    ...
    assert "temporarily disabled" in watchdog.get_disabled_message("test_tool", "test_action").lower()
...
def test_runtime_memory_note_persistence():
    ...
    with patch.object(jarvis, '_speak'), \
         patch.object(jarvis, '_save_mem') as mock_save:
        
        result = jarvis._handle_note("note My focus niche this week is fitness creators")
